...
import {Event, EventTypes, Game, Player, User, EventStatus} from "../bdd/entities";
...
router.post("/create", authVerification, async function(req, res, next) {
  ...
  const event = await BddService.eventHandler.createEvent(req.body, player, user, game, eventType, eventStatus);
...
router.get("/event/:id", async function(req, res, next) {
  ...
  const event = <Event> await BddService.eventHandler.findEventById(idEvent);
  return res.status(200).send(event);
});
